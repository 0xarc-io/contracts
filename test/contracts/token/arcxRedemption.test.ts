...
describe('ArcxRedemption', () => {
  ...
  before(async () => {
    ...
    await new TestTokenFactory(user)
      .attach(arcxToken.address)
      .approve(redemption.address, ARCX_AMOUNT);
  });
  ...
  describe('#redeem', function () {
    it('should not be able to redeem without enough ARCx', async () => {
      await expect(redemption.redeem(ARCX_AMOUNT.add(1))).to.revertedWith(
    ...
    it('should not be able to redeem past the redemption date', async () => {
      ...
      await expect(redemption.redeem(ARCX_AMOUNT)).to.revertedWith(
  ...
  describe('#withdraw', function () {
    it('should not be able to withdraw as a non-owner', async () => {
      await expect(
        redemption.withdraw(usdcToken.address, ARCX_AMOUNT, user.address),
      ).to.be.revertedWith('Ownable: caller is not the owner');
    });

    it('should be able to withdraw as the owner', async () => {
      const ownerRedemption = new MockArcxRedemptionFactory(owner).attach(
        redemption.address,
      );
      ...
      expect(await usdcToken.balanceOf(redemption.address)).to.eq(
        USDC_AMOUNT.div(2),
      );
      expect(await arcxToken.balanceOf(redemption.address)).to.eq(ARCX_AMOUNT);

      await ownerRedemption.withdraw(
        usdcToken.address,
        USDC_AMOUNT.div(2),
        user.address,
      );
