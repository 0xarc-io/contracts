...
describe('StakingAccrualERC20', () => {
  ...
  before(async () => {
    ...
    await starcx.init('stARCx', 'stARCx', 18, stakingToken.address);
  });
  ...
  describe('Mutating functions', () => {
    ...
    describe('#exit', () => {
      ...
      it(
        'exits with MORE ARCx than initially if the contract has accumulated more tokens',
      );

      it(
        'exits with LESS ARCx than initially if the admnin had removed tokens',
      );
    });
