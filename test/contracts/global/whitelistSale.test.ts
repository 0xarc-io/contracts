...
describe('WhitelistSale', () => {
  ...
  describe('#setAllocation', () => {
    ...
    it('should be able to remove an allocation successfully', async () => {
      ...
      const participant1New = await whitelistSale.participants(
        signers[1].address,
      );
      const participant2New = await whitelistSale.participants(
        signers[2].address,
      );
  ...
  describe('#claimAllocation', () => {
    ...
    it('should transfer the funds from user to owner', async () => {
      ...
      await expect(() =>
        userWhiteSaleContract.claimAllocation(claimAmount),
      ).to.changeTokenBalance(currency, ownerAccount, claimAmount);
    });
  });
