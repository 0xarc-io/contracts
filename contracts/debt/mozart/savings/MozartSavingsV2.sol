...
contract MozartSavingsV2 is Adminable, MozartSavingsStorage, IERC20 {
        ...
        returns (uint256)
    {
     ...
     function accumulatedInterest()
        ...
        returns (uint256)
    {
        // First we multiply the savings rate (expressed in rate/sec) by the time since
        // the last update. This result represents the proportional amount of interest to
        // apply to the system at a whole
        return savingsRate.mul(currentTimestamp().sub(indexLastUpdate));
    }
        ...
        returns (uint256)
    {
        ...
        uint256 totalInterestAccumulated = accumulatedInterest();
        ...
        returns (uint256)
    {
        ...
        // Update the new savingsIndex directly
        savingsIndex = newSavingsIndex;

        // Update the total supplied amount based on the proportional rate of interest
        // applied to the entire system
        totalSupplied = totalSupplied.mul(growthRate).div(BASE);
