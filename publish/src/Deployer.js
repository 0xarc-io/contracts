...
const { Signer, providers, ethers } = require('ethers');
...
class Deployer {
	...
	constructor({
		...
		this.provider = new ethers.providers.JsonRpcProvider(providerUrl, this.network);
	...
	async _deploy({ name, source, args = [], deps = [], force = false, dryRun = this.dryRun }) {
		...
		if (deploy) {
			...
			if (dryRun) {
				...
				const newContract = new ethers.ContractFactory(compiled.abi, '0x' + bytecode, this.account);

				let deployTx = await newContract.getDeployTransaction();
				deployTx = {
					...deployTx,
					...this.sendParameters('contract-deployment'),
				};

				const tx = await this.account.sendTransaction(deployTx);
