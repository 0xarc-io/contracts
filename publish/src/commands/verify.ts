...
const verify = async ({ buildPath, network, deploymentPath }) => {
  ...
  async function verifyContract(name, source, address) {
    ...
    if (result.data.result === 'Contract source code not verified') {
      ...
      const readDeployedByteCode = () => {
        const flattenedFilename = path.join(buildPath, '..', 'artifacts', `${source}.json`);
        try {
          return JSON.parse(fs.readFileSync(flattenedFilename)).deployedBytecode;
        } catch (err) {
      ...
      const compiledBytecode = readDeployedByteCode().slice(-150);
      ...
      const readFlattened = () => {
        const flattenedFilename = path.join(
          buildPath,
          FLATTENED_FOLDER,
          Object.keys(deployment.sources[name].metadata.sources)[0],
        );
        console.log(flattenedFilename);

        // glob(path.join(buildPath, FLATTENED_FOLDER) + `/**/*.sol`, {}, (err, files) => {
  ...
  await asyncForEach(Object.keys(config), async (synthName) => {
    const { address, source } = deployment.targets[synthName];
    await verifyContract(synthName, source, address);

    await asyncForEach(Object.keys(config[synthName].dependencies), async (name) => {
      console.log(name);
      const { address, source } = deployment.targets[synthName].dependencies[name];
      await verifyContract(name, source, address);
    });
  });
